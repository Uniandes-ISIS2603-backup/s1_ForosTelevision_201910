...
public class Productora {
    private static final String COLLECTION = "ProductoraResource.postman_collection";

    @Deployment(testable = true)
    public static WebArchive createDeployment() {
        return ShrinkWrap.create(WebArchive.class, "s1_foros-api.war")//War del modulo api
                // Se agrega las dependencias
                .addAsLibraries(Maven.resolver().loadPomFromFile("pom.xml")
                        .importRuntimeDependencies().resolve()
                        .withTransitivity().asFile())
                // Se agregan los compilados de los paquetes de servicios
                .addPackage(co.edu.uniandes.csw.foros.resources.RestConfig.class.getPackage()) //No importa cual recurso usar, lo importante es agregar el paquet
